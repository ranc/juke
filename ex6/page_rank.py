...
def rank_pages(traffic_dict: Dict[str, Dict[str, int]], iterations) -> Dict[str, int]:
    total_links_from = calc_links_from(traffic_dict)
    page_ranks = dict((page, 1) for page in total_links_from.keys())
    for _ in range(iterations):
        new_page_ranks = {}
        for page_j in traffic_dict.keys():
            new_page_ranks[page_j] = 0
            for page_i, link_i_to_all in traffic_dict.items():
